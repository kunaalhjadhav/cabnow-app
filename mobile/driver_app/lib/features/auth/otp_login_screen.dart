...
class _OtpLoginScreenState extends State<OtpLoginScreen> {
  ...
  Future<void> _verifyOtp() async {
    ...
    try {
      await context.read<AuthState>().verifyOtp(_phoneController.text.trim(), _code, fullName: _nameController.text.trim());
      if (mounted) {
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Text('Driver Partner', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Text(_otpSent ? 'Enter the OTP sent to ${_phoneController.text}' : 'Sign in with your phone number',
                  style: const TextStyle(color: Colors.black54)),
              const SizedBox(height: 24),
              if (!_otpSent) ...[
                TextField(controller: _phoneController, keyboardType: TextInputType.phone, decoration: const InputDecoration(labelText: 'Phone number', hintText: '+919876543210', border: OutlineInputBorder())),
                const SizedBox(height: 12),
                TextField(controller: _nameController, decoration: const InputDecoration(labelText: 'Full name (first-time signup only)', border: OutlineInputBorder())),
              ] else
                PinCodeTextField(
                  appContext: context,
                  length: 6,
